...
class IMAPServerOptions(Options):
    synopsis = "IMAPserver.py [options]"
    ...
    def postOptions(self):
        ...
        if self["port"] is None:
            raise UsageError("Must specify a port number with -p or --port")
        try:
            self["port"] = int(self["port"])
        except ValueError:
            raise UsageError("Port argument must be a number.")
        if self["mail-storage"] is None:
            raise UsageError("Must specify a mail-storage path with -s or --mail-storage")
...
reactor.listenTCP(o["port"], IMAPFactory(portal))
reactor .run()